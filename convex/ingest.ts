...
const normalizeImportItem = (item: {
  ...
  if (!(question && answer)) {
    throw new Error(
      "Each QA item must provide non-empty question and answer fields."
    );
  }
  ...
  const searchFields = (
    item.search_fields && typeof item.search_fields === "object"
      ? item.search_fields
      : {}
  ) as Record<string, unknown>;
  ...
  const sectionNumber = normalizeString(
    item.section_number ?? metadata.section_number
  );
  const sectionTitle = normalizeString(
    item.section_title ?? metadata.section_title
  );
  const questionNumber = normalizeString(
    item.question_number ?? metadata.question_number
  );
  const sourceId =
  ...
  const keywords =
    collectKeywords(metadata.keywords) ?? collectKeywords(item.keywords);
        ...
        .map((source) => {
          ...
          if (source && typeof source === "object") {
            ...
            const location = normalizeString(
              (source as Record<string, unknown>).location
            );
            const note = normalizeString(
              (source as Record<string, unknown>).note
            );
            if (type) {
              normalizedSource.type = type;
            }
            if (title) {
              normalizedSource.title = title;
            }
            if (url) {
              normalizedSource.url = url;
            }
            if (location) {
              normalizedSource.location = location;
            }
            if (note) {
              normalizedSource.note = note;
            }
            return Object.keys(normalizedSource).length > 0
              ? normalizedSource
              : undefined;
          }
          return undefined;
  ...
  const questionLower =
    normalizeString(searchFields.question_lower) ?? question.toLowerCase();
