...
export function usePaginatedQuestions(lang?: string, initialSection?: string) {
  const initialSectionNumber = useMemo(() => {
    const parsed = Number.parseInt(initialSection ?? "1", 10);
  ...
  const currentSectionIndex = useMemo(() => {
    return availableSections.findIndex(
      (s) => s.section_number === currentSectionNumber
    );
  ...
  const result = useQuery(api.qa.getPaginated, {
    paginationOpts: {
  ...
  const loadNextSection = useCallback(() => {
    if (!hasMoreSections || currentSectionIndex < 0) {
      return;
    }
  ...
  const sectionsArray = useMemo(() => {
    return Object.values(sections).sort(
      (a, b) =>
        (Number.parseInt(a.section_number, 10) || 0) -
        (Number.parseInt(b.section_number, 10) || 0)
    );
  }, [sections]);

  // Current section state
  const currentSectionState = sections[currentSectionNumber];
  const hasMoreCurrentSection =
    currentSectionState?.hasMore ?? (result ? !result.isDone : true);

  // Loading states - use tracked state to avoid flicker
  const isLoadingInitial =
    !sectionsInfo || (sectionsArray.length === 0 && !result);
  const isLoadingCurrentSection =
    isLoadingMore || (!result && sectionsArray.length > 0);
